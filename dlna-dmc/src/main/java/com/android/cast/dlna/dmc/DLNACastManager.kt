...
object DLNACastManager : OnDeviceRegistryListener {
    ...
    // -----------------------------------------------------------------------------------------
    private val deviceListeners: MutableList<OnDeviceRegistryListener> = ArrayList()
    ...
    fun unregisterDeviceListener(listener: OnDeviceRegistryListener) {
        deviceListeners.remove(listener)
    }

    override fun onDeviceAdded(device: Device<*, *, *>) {
        if (checkDeviceType(device)) {
            deviceListeners.forEach { listener -> listener.onDeviceAdded(device) }
        }
    }
    ...
    override fun onDeviceRemoved(device: Device<*, *, *>) {
        ...
        if (checkDeviceType(device)) {
            deviceListeners.forEach { listener -> listener.onDeviceRemoved(device) }
        }
    }
    ...
    fun search(type: DeviceType? = null) {
        upnpService?.get()?.also { service ->
            ...
            service.registry.devices?.filter { searchDeviceType == null || searchDeviceType != it.type }?.onEach {
                ...
                deviceListeners.forEach { listener -> listener.onDeviceRemoved(it) }
