...
import com.android.cast.dlna.core.didl.DIDLBuilder
...
internal abstract class BaseServiceExecutor(
    ...
    internal class AVServiceExecutorImpl(
        ...
        override fun setAVTransportURI(uri: String, title: String, callback: ServiceActionCallback<Unit>?) {
            ...
            val metadata = DIDLBuilder.buildVideo(uri, title)
        ...
        override fun seek(milliseconds: Long, callback: ServiceActionCallback<Unit>?) {
            logger.i("${Actions.Seek}: ${ModelUtil.toTimeString(milliseconds / 1000)}")
            if (invalidServiceAction(Actions.Seek)) {
                notifyFailure(callback)
                return
            }
            executeAction(object : Seek(service, getStringTime(milliseconds)) {
                override fun success(invocation: ActionInvocation<*>?) {
                    notifySuccess(callback, result = Unit)
                }
