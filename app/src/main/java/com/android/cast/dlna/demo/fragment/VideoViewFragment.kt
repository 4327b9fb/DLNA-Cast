...
import com.android.cast.dlna.demo.http.LocalServer
...
class VideoViewFragment : Fragment(), OnKeyEventHandler {
    ...
    private var durationMilliseconds: Long = 0
        ...
        deviceControl.getPositionInfo(object : ServiceActionCallback<PositionInfo> {
            override fun onSuccess(result: PositionInfo) {
                if (durationMilliseconds == 0L) {
                    durationMilliseconds = result.trackDurationSeconds * 1000
                }
    ...
    private val seekBarChangeListener: OnSeekBarChangeListener = object : OnSeekBarChangeListener {
        ...
        override fun onStopTrackingTouch(seekBar: SeekBar) {
            if (durationMilliseconds > 0) {
                deviceControl.seek(seekBar.progress * durationMilliseconds / seekBar.max)
            }
        }
    }
        ...
        if (requestCode == 987 && resultCode == Activity.RESULT_OK) {
            data?.data?.also { uri ->
                ...
                if (file == null) {
                    ...
                    if (localServer == null) {
                        localServer = LocalServer(requireContext())
                    }
                    localServer?.startServer()
                    durationMilliseconds = 0L
                    deviceControl.setAVTransportURI("${localServer?.baseUrl}${file.path}", file.nameWithoutExtension, object : ServiceActionCallback<Unit> {
                        override fun onSuccess(result: Unit) {
                            positionHandler.start()
                        }
