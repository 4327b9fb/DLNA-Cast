...
/**
 * StreamClient implementation based on OkHttp 4.x/5.x.
 *
 * OkHttp 4.x+ uses Kotlin-style API with extension functions:
 * - [MediaType.Companion.toMediaType] instead of [MediaType.parse]
 * - [RequestBody.Companion.toRequestBody] instead of [RequestBody.create]
 * - Property access: [response.code], [response.message], [response.headers]
 * - [client.dispatcher] property instead of [client.dispatcher()] method
 *
 * Requires Android API 21+ and Okio 3.x.
...
class OkHttp4StreamClient(
    ...
    override fun createRequest(requestMessage: StreamRequestMessage): okhttp3.Call? {
        ...
        if (hasBody) {
            ...
            // OkHttp 4.x: use toMediaType() extension function
            val contentType = "text/xml; charset=\"utf-8\"".toMediaType()
            // OkHttp 4.x: use toRequestBody() extension function
            val requestBody = (bodyBytes ?: ByteArray(0)).toRequestBody(contentType)
    ...
    override fun createCallable(
        ...
        return Callable {
            ...
            try {
                ...
                // OkHttp 4.x: property access instead of method calls
                ...
                // Headers — OkHttp 4.x: response.headers is a property (Headers object)
    ...
    override fun stop() {
        logger.trace("Shutting down OkHttp4 StreamClient")
        // OkHttp 4.x: dispatcher is a property, executorService is a property
